...
def load_model(model_path):
    try:
        from mlx_embeddings import load, generate as mlx_generate
        import numpy as np

        model, tokenizer = load(model_path)

        def fn(texts):
            # mlx_embeddings.generate returns output.text_embeds, already L2-normalised.
            output = mlx_generate(model, tokenizer, texts=texts)
            arr = np.array(output.text_embeds)
            return arr.tolist()

        return fn
    except ImportError:
        print(
            "mlx-embeddings not found — install with: pip install mlx-embeddings",
            file=sys.stderr,
            flush=True,
        )
        sys.exit(1)
