...
func PrintLastLogLines(logFile string, n int) {
	...
	fmt.Fprintf(os.Stderr, "\n%s\n", color.Gra5("--- last log lines ---"))
	for _, l := range lines {
		fmt.Fprintf(os.Stderr, "  %s\n", l)
	}
	fmt.Fprintf(os.Stderr, "%s\n", color.Gra5("--- full log: "+logFile+" ---"))
...
func Stop(modelID string) error {
	...
	if state == nil {
		fmt.Printf("  %s  %s\n", modelID, color.Gra5("not running"))
		return nil
	}
	if !PidAlive(state.PID) {
		fmt.Printf("  pid %-7d  %s  %s\n", state.PID, modelID, color.Gra5("already stopped (stale state removed)"))
		RemoveState(modelID)
		return nil
	}
	...
	RemoveState(modelID)
	fmt.Printf("  pid %-7d  %s  %s\n", state.PID, modelID, color.Gra5("stopped"))
	return nil
}
